    ...
    if id_token:
        try:
            ...
            if not any(existing_room):
                # If no room with the same name exists, add the room
                ev_data = {
                    'name': name,
                ...
                # If a room with the same name already exists, return a message or handle it accordingly
...
async def delete_room(room_id: str, request: Request):
    ...
    # Get all bookings for the specified room_id
    bookings_ref = firestore_db.collection('bookings').where('room_id', '==', room_id).get()
    
    # Flag to determine if any booking matched the current date
    date_matched = False
    
    # Check for current date in end_date of each booking
    for booking in bookings_ref:
        booking_data = booking.to_dict()
        print(booking_data)
        if booking_data['end_date'] == current_date or booking_data['end_date'] > current_date:
            # Perform additional action if the end_date matches the current date
            print('date matched')
            date_matched = True
            break  # No need to continue checking other bookings
    
    # If no booking matched the current date, and user_id is matched, delete the room
    if not date_matched and user_id:
        # Check if the user_id matches the stored user_id in rooms table for that room
...
async def edit_booking(request: Request, room_name: str):
    ...
    if room_id:
        ...
        # If user is authenticated, filter bookings by user_id
        if user_id:
            bookings_ref = bookings_ref.where('user_id', '==', user_id)
